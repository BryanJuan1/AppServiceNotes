...
def MainPage(page: ft.Page):
    global switchButton_Parts
    switchButton_Parts = None
    ...
    def toggle_DisplayPart(e):
        ...
        if not e.control.value and parts_Expanded:
            toggle_parts(None)
        page.update()
    ...
    def update_menu_Parts():
        ...
        global switchButton_Parts
        switch_value = switchButton_Parts.value if switchButton_Parts else False

        global button_Parts
        button_Parts = ft.ElevatedButton(text='Peças', on_click=toggle_parts, disabled=not switch_value)
        switchButton_Parts  = ft.Switch(label='', value=switch_value, on_change=toggle_DisplayPart)
        
        row_PartsControls = ft.Row(controls=[button_Parts,switchButton_Parts], alignment=ft.MainAxisAlignment.CENTER)
    ...
    def EnviarInfo(e):
        ...
        if switchButton_Parts and switchButton_Parts.value:
            #Extrair Valores Peças
            for row in parts_fieldsList:
                text_Parts = row.controls[0].value
                text_Valor = row.controls[1].value
                if text_Parts != "":
                    parts_Info.append(f'{text_Parts}: R$ {text_Valor}')
        ...
        for row in workHand_fieldsList:
            ...
            text_Valor = row.controls[1].value
            if text_Service != "":
                workHand_Info.append(f'{text_Service}: R$ {text_Valor}')

        other_info = [
            titleServiceNotes.value,
            infoWorkshop.value,
            infoContact.value,
            editable_textMechanic.value,
            infoDate.value,
            f'• Carro: {checkText_Car}'
        ]
        ...
        if workHand_Info:
            print('Mão de Obra:')
            for item in workHand_Info:                
                print(f'- {item}')
            
        else:
            print("Mão de Obra: Nenhuma informação inserida")
        if parts_Info:
            print('Peças:')
    ...
    columnMain_WorkHand     = ft.Column(expand=False, wrap=True, alignment= ft.MainAxisAlignment.CENTER) # Cria a coluna principal para adicionar elementos
    columnMain_Parts        = ft.Column(expand=True, wrap=True, alignment= ft.MainAxisAlignment.CENTER) # Cria a coluna principal para adicionar elementos
